...
public class SchematicUtil {
    ...
    public static void createCustomSchematic(World world, Location3I loc1, Location3I loc2, String fileName) {
        ...
        for (int y = smallY; y < largeY; y++) {
            for (int x = smallX; x < largeX; x++) {
                for (int z = smallZ; z < largeZ; z++) {
                    ...
                    String blockData = GameData.getBlockRegistry()
                        .getNameForObject(b);
        ...
        for (CustomEntry<String, List<Integer>> i : tileEntityData) {
            ...
            for (int id : i.getValue()) tileEntityBlockIds.add(new JsonPrimitive(id));
        ...
        FileUtil.writeToFile(
            ConfigLoader.folder.getAbsolutePath() + "/CustomRewards/Schematics/" + fileName,
            gson.toJson(json));
    }

    public static CustomSchematic loadCustomSchematic(String file, int xOffSet, int yOffSet, int zOffSet, float delay,
        boolean falling, boolean relativeToPlayer) {
        JsonElement elem = FileUtil
            .readJsonfromFile(ConfigLoader.folder.getAbsolutePath() + "/CustomRewards/Schematics/" + file);
        if (elem == null) return null;
        JsonObject json = elem.getAsJsonObject();
        List<OffsetBlock> offsetBlocks = new ArrayList<OffsetBlock>();
        JsonObject info = json.get("Schematic Data")
            .getAsJsonObject();
        int xSize = info.get("xSize")
            .getAsInt();
        int ySize = info.get("ySize")
            .getAsInt();
        int zSize = info.get("zSize")
            .getAsInt();
        List<CustomEntry<Integer, String>> blockDataIds = new ArrayList<CustomEntry<Integer, String>>();

        JsonArray blockDataArray = json.get("Block Data")
            .getAsJsonArray();
        for (JsonElement i : blockDataArray) {
            JsonObject index = i.getAsJsonObject();
            for (Entry<String, JsonElement> obj : index.entrySet()) blockDataIds.add(
                new CustomEntry<Integer, String>(
                    obj.getValue()
                        .getAsInt(),
                    obj.getKey()));
        }
        ...
        float delayTotal = 0;
        int index = 0;
        JsonArray blockArray = json.get("Blocks")
            .getAsJsonArray();
        for (int yOff = 0; yOff < ySize; yOff++) {
            for (int xOff = (xSize / 2) - xSize; xOff < (xSize / 2); xOff++) {
                for (int zOff = (zSize / 2) - zSize; zOff < (zSize / 2); zOff++) {
                    ...
                    if (!b.equals(Blocks.air)) delayTotal += delay;
                    index++;
                }
            }
        }

        JsonArray teArray = json.get("TileEntities")
            .getAsJsonArray();
        for (JsonElement i : teArray) {
            for (Entry<String, JsonElement> obj : i.getAsJsonObject()
                .entrySet()) {
                String teData = obj.getKey();
                for (JsonElement ids : obj.getValue()
                    .getAsJsonArray()) {
                    int id = ids.getAsInt();
                    OffsetBlock osb = offsetBlocks.get(id);
                    OffsetTileEntity oste = OffsetBlockToTileEntity(osb, teData);
                    if (oste != null) offsetBlocks.set(id, oste);
                }
            }
        }
        ...
        for (int i = offsetBlocks.size() - 1; i >= 0; i--) {
            ...
            if (osb.getBlock()
                .equals(Blocks.air)) offsetBlocks.remove(i);
        }
    ...
    public static OffsetTileEntity OffsetBlockToTileEntity(OffsetBlock osb, NBTTagCompound nbt) {
        OffsetTileEntity oste = new OffsetTileEntity(
            osb.xOff,
            osb.yOff,
            osb.zOff,
            osb.getBlock(),
