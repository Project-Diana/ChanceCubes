...
public class WorldGen {
    ...
    public void onGenerate(PopulateChunkEvent.Pre event) {
        if (CCubesSettings.isBlockedWorld(
            event.world.getWorldInfo()
                .getWorldName())
            || CCubesSettings.isBlockedWorld("" + event.world.provider.dimensionId)) return;
        if (CCubesSettings.oreGeneration) generateOre(event.world, new Random(), event.chunkX * 16, event.chunkZ * 16);
    ...
    private void generateOre(World world, Random rand, int x, int z) {
        for (int k = 0; k < CCubesSettings.oreGenAmount; k++) {
            ...
            (new WorldGenMinable(CCubesBlocks.chanceCube, 3))
    ...
    public void generateSurface(World world, Random rand, int x, int z) {
        if (rand.nextInt(100) < CCubesSettings.surfaceGenAmount) {
            ...
            if (world.getBlock(xCord, yCord - 1, zCord)
                .equals(Blocks.bedrock)) {
                for (int y = 0; y < yCord; y++) {

                    if (world.getBlock(xCord, y - 1, zCord)
                        .isBlockSolid(world, xCord, y, zCord, 1) && world.isAirBlock(xCord, y, zCord)) {
                        yCord = y;
                        return;
                    }
                }
