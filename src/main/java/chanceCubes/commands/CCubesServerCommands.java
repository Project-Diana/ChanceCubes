...
public class CCubesServerCommands implements ICommand {
    ...
    public void processCommand(ICommandSender icommandsender, String[] astring) {
        if (astring.length > 0 && astring[0].equalsIgnoreCase("reload")) {
            ...
            if (icommandsender instanceof EntityPlayer) {
                ...
                if (player.inventory.getCurrentItem() != null) {
                    ...
                    if (nbt != null) icommandsender.addChatMessage(new ChatComponentText(nbt.toString()));
                    else icommandsender.addChatMessage(new ChatComponentText("This item has no tag nbt data"));
                }
            }
            ...
            if (icommandsender instanceof EntityPlayer) {
                ...
                if (stack != null) {
                    String info = GameData.getItemRegistry()
                        .getNameForObject(stack.getItem());
                    if (info == null || info == "") info = GameData.getBlockRegistry()
                        .getNameForObject(stack.getItem());
            ...
            if (astring.length > 1) {
                if (ChanceCubeRegistry.INSTANCE.unregisterReward(astring[1])) icommandsender
                    .addChatMessage(new ChatComponentText(astring[1] + " Has been temporarily disabled."));
                else icommandsender.addChatMessage(
                    new ChatComponentText(
                        astring[1] + " is either not currently enabled or is not a valid reward name."));
            } else {
            ...
            if (astring.length > 1) {
                if (ChanceCubeRegistry.INSTANCE.enableReward(astring[1]))
                    ...
                    new ChatComponentText(
                        astring[1] + " is either not currently disabled or is not a valid reward name."));
            } else {
        ...
        if (astring[0].equalsIgnoreCase("schematic")) {
            if (Minecraft.getMinecraft()
                .isSingleplayer()) {
                if (icommandsender instanceof EntityPlayer) {
                    World world = Minecraft.getMinecraft()
                        .getIntegratedServer()
                        .getEntityWorld();
                    EntityPlayer player = (EntityPlayer) icommandsender;
                    if (player.capabilities.isCreativeMode) {
                        if (astring.length >= 3) {
                            if (astring[1].equalsIgnoreCase("setPoint")) {
                                if (astring[2].equalsIgnoreCase("1")) {
                                    SchematicUtil.selectionPoints[0] = new Location3I(
                                        (int) player.posX,
                                        (int) player.posY - 1,
                                        (int) player.posZ);
                                ...
                                if (astring[2].equalsIgnoreCase("2")) {
                                    SchematicUtil.selectionPoints[1] = new Location3I(
                                        (int) player.posX,
                                        (int) player.posY - 1,
                                        (int) player.posZ);
                                ...
                                if (SchematicUtil.selectionPoints[0] == null
                                    || SchematicUtil.selectionPoints[1] == null) {
                                    icommandsender.addChatMessage(new ChatComponentText("Both points are not set!"));
                                    return;
                                }
                                SchematicUtil.createCustomSchematic(
                                    world,
                                    SchematicUtil.selectionPoints[0],
                                    SchematicUtil.selectionPoints[1],
                                    astring[2].endsWith(".ccs") ? astring[2] : astring[2] + ".ccs");
                                icommandsender.addChatMessage(
                                    new ChatComponentText(
                                        "Schematic file named "
                                            + (astring[2].endsWith(".ccs") ? astring[2] : astring[2] + ".ccs")
                                            + " created!"));
                        ...
                        icommandsender.addChatMessage(
                            new ChatComponentText("Sorry, you need to be in creative to use this command"));
                    }
                }
            } else {
                icommandsender
                    .addChatMessage(new ChatComponentText("Sorry, but this command only works in single player"));
            }
    ...
    public List addTabCompletionOptions(ICommandSender icommandsender, String[] astring) {
        if (astring.length == 0) return tab;
        return null;
    }
