...
public class ThermalFoundationModHook extends BaseModHook {
    ...
    private String[] rodTypes = { "Platinum", "Electrum", "Invar", "Tin", "Lead", "Nickel", "Silver", "Bronze",
        "Copper" };
    ...
    public void loadRewards() {
        ...
        if (GameRegistry.findItem(super.modId, "material") != null) {
            ...
            ChanceCubeRegistry.INSTANCE.registerReward(
                new BasicReward(
                    this.modId + ":Grind_Gears",
                    10,
                    new MessageRewardType(new MessagePart("You really grind my gears ;)")),
                    new ItemRewardType(new ItemPart(stack))));
        }

        List<ItemPart> avialable = new ArrayList<ItemPart>();
        for (String s : this.rodTypes) if (GameRegistry.findItem(super.modId, "tool.fishingRod" + s) != null)
            avialable.add(new ItemPart(GameRegistry.findItemStack(super.modId, "tool.fishingRod" + s, 1)));
        ChanceCubeRegistry.INSTANCE.registerReward(
            new BasicReward(
                this.modId + ":fishing",
                5,
                new MessageRewardType(new MessagePart("Hey! Let's open a fishing store!")),
                new ItemRewardType(avialable.toArray(new ItemPart[avialable.size()]))));
    }
