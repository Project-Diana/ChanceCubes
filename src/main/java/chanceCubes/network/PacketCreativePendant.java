...
public class PacketCreativePendant implements IMessage {
    ...
    public static final class Handler implements IMessageHandler<PacketCreativePendant, IMessage> {
        ...
        public IMessage onMessage(PacketCreativePendant message, MessageContext ctx) {
            Container c;
            try {
                c = MinecraftServer.getServer()
                    .getEntityWorld()
                    .getPlayerEntityByName(message.playerName).openContainer;
            } catch (Exception NullPointerException) {
                CCubesCore.logger.log(
                    Level.ERROR,
                    "Chance Cubes has failed to set the chance of a cube due to a packet failure! Please Inform Turkey of this!");
                return null;
            }
