...
public class CustomExtendedList extends GuiListExtended {
    ...
    public CustomExtendedList(ConfigGui parentScreen, Minecraft mc, int p_i45010_2_, int p_i45010_3_, int p_i45010_4_,
    ...
    protected void keyTyped(char p_73869_1_, int p_73869_2_) {
        for (IGuiListEntry entry : elements)
            if (entry instanceof CustomTextEntry) ((CustomTextEntry) entry).keyTyped(p_73869_1_, p_73869_2_);
    }
    ...
    public boolean func_148179_a(int x, int y, int mouseEvent) {
        ...
        for (IGuiListEntry entry : elements)
            if (entry instanceof CustomTextEntry) ((CustomTextEntry) entry).mousePressed(0, x, y, mouseEvent, 0, 0);
        return true;
    }
    ...
    public class CustomListEntry implements IGuiListEntry {
        ...
        public boolean mousePressed(int index, int x, int y, int mouseEvent, int relativeX, int relativeY) {
            if (this.button.mousePressed(mc, x, y)) parentScreen.nextEditStage(button.id, button.displayString);
            return false;
        }
