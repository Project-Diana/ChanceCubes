...
public class ConfigGui extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList
            .add(this.buttonNew = new GuiButton(1, this.width / 2 - 50, this.height - 25, 100, 20, buttonNewText));
        this.buttonList
            .add(this.buttonCancel = new GuiButton(2, this.width / 2 - 50, this.height - 55, 100, 20, "Cancel"));
        this.buttonList.add(this.buttonSave = new GuiButton(3, this.width / 2 - 50, this.height - 25, 100, 20, "Save"));
        // this.buttonList.add(this.buttonDelete = new GuiButton(4, this.width / 2 - 175, this.height - 25, 98, 20,
        // "Delete"));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.isCreatingNew) this.textFieldNew.drawTextBox();
        if (this.isDeleting) mc.fontRenderer.drawString(
            this.deleteWarnText,
            this.width / 2 - (int) (deleteWarnText.length() * 2.5),
            this.height / 2,
            0xFF0000);
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.enabled) {
            if (button.id == 0) {
                ...
                if (this.isCreatingNew) {
                    ...
                    switch (editState) {
                        case Files: {
                            ...
                            file = new File(
                                CustomRewardsLoader.instance.getFolderFile()
                                    .getAbsolutePath() + "/" + fileName);
                            try {
                                file.createNewFile();
                            } catch (IOException e) {
                                CCubesCore.logger
                                    .log(Level.ERROR, "Failed to create the new file for a custom reward!");
                                e.printStackTrace();
                                return;
                            }
                            break;
                        ...
                        case All_Rewards: {
                            ...
                            file = new File(
                                CustomRewardsLoader.instance.getFolderFile()
                                    .getAbsolutePath() + "/" + this.prevStage[0]);
                            if (!file.exists()) {
                            ...
                            fileJson.getAsJsonObject()
                                .add(rewardName, newRewardJson);
                            break;
                        }
                        case Single_Reward: {
                            ...
                            fileJson = json.parse(this.loadFileContents(file));
                            JsonElement rewardJson = fileJson.getAsJsonObject()
                                .get(this.prevStage[1]);
                            if (rewardJson == null) {
                                CCubesCore.logger.log(
                                    Level.ERROR,
                                    "Failed to find the reward with the given name of " + this.prevStage[1]);
                                return;
                            }

                            JsonArray rewardTypeArray = new JsonArray();
                            rewardJson.getAsJsonObject()
                                .add(rewardType, rewardTypeArray);
                            break;
                        }
                        case Reward_Type: {
                            ...
                            JsonArray rewardTypeArray = rewardJson.getAsJsonObject()
                                .get(this.prevStage[2])
                                .getAsJsonArray();
                ...
                if (rewardJson == null) {
                    CCubesCore.logger
                        .log(Level.ERROR, "Failed to find the reward with the given name of " + this.prevStage[1]);
                    return;
                }

                if (this.entries.getListEntry(0) instanceof CustomTextEntry
                    && ((CustomTextEntry) this.entries.getListEntry(0)).getLabel()
                        .equalsIgnoreCase("Chance:")) {
                    CustomTextEntry textBox = (CustomTextEntry) this.entries.getListEntry(0);
                    try {
                        int chance = Integer.parseInt(
                            textBox.getTextBox()
                                .getText());
                        ...
                        CCubesCore.logger
                            .log(Level.ERROR, "Failed to cast the chance value of the reward to an integer");
                        return;
                    }
                    try {
                        FileOutputStream outputStream = new FileOutputStream(file);
    ...
    public void reloadStage() {
        switch (editState) {
            ...
            case Edit_Reward_Type: {
                this.loadSpecificRewardType(
                    this.prevStage[3].substring(this.prevStage[3].indexOf(":") + 1),
                    this.prevStage[2]);
                break;
            }
        }
    ...
    public void loadFiles() {
        ...
        for (String s : CustomRewardsLoader.instance.getRewardsFiles()) entries.addButton(s);
    }
    ...
    public void loadFileRewards(String file) {
        ...
        entries.clearElements();
        for (String s : CustomRewardsLoader.instance.getRewardsFromFile(file)) entries.addButton(s);
    }
    ...
    public void loadSingleReward(String reward) {
        ...
        entries.clearElements();
        for (String s : CustomRewardsLoader.instance.getReward(this.prevStage[0], reward)) entries.addButton(s);
    }
    ...
    public void loadRewardType(String type) {
        ...
        if (type.equalsIgnoreCase("Chance")) {
            ...
            entries.addTextEntry(
                "Chance",
                CustomRewardsLoader.instance.getRewardType(this.prevStage[0], this.prevStage[1], type)
                    .get(0));
        } else {
    ...
    public void loadSpecificRewardType(String rawJson, String type) {
        ...
        try {
            convertedJson = json.parse(rawJson)
                .getAsJsonObject();
        } catch (Exception e) {
        ...
        if (type.equalsIgnoreCase("Item")) this.loadRewardPart(convertedJson, ItemPart.elements);
        else if (type.equalsIgnoreCase("Block")) this.loadRewardPart(convertedJson, OffsetBlock.elements);
        else if (type.equalsIgnoreCase("Message")) this.loadRewardPart(convertedJson, MessagePart.elements);
        else if (type.equalsIgnoreCase("Command")) this.loadRewardPart(convertedJson, CommandPart.elements);
        else if (type.equalsIgnoreCase("Entity")) this.loadRewardPart(convertedJson, EntityPart.elements);
        else if (type.equalsIgnoreCase("Experience")) this.loadRewardPart(convertedJson, ExpirencePart.elements);
        else if (type.equalsIgnoreCase("Potion")) this.loadRewardPart(convertedJson, PotionPart.elements);
        else if (type.equalsIgnoreCase("Schematic"))
            this.loadRewardPart(convertedJson, new String[] { "fileName:S", "delay:I" });
        else if (type.equalsIgnoreCase("Sound")) this.loadRewardPart(convertedJson, SoundPart.elements);
        else if (type.equalsIgnoreCase("Chest")) this.loadRewardPart(convertedJson, ChestChanceItem.elements);
        else if (type.equalsIgnoreCase("Particle")) this.loadRewardPart(convertedJson, ParticlePart.elements);
    ...
    public void loadRewardPart(JsonObject convertedJson, String[] parts) {
        for (String s : parts) {
            ...
            String value = "";
            if (convertedJson.has(s)) value = convertedJson.get(s)
                .getAsString();
    ...
    public void saveRewardPart(File file, JsonElement fileJson, JsonElement rewardJson) {
        JsonArray rewardTypeArray = rewardJson.getAsJsonObject()
            .get(this.prevStage[2])
            .getAsJsonArray();
        JsonObject rewardInstance = rewardTypeArray
            .get(Integer.parseInt(this.prevStage[3].substring(0, this.prevStage[3].indexOf(":"))))
            .getAsJsonObject();
        ...
        for (int i = 0; i < this.entries.getSize(); i++) {
            ...
            if (entry instanceof CustomTextEntry) {
                ...
                try {
                    for (String s : rewardElements) {
                        ...
                        String label = textBox.getLabel()
                            .substring(
                                ...
                                textBox.getLabel()
                                    .length() - 1);
                        String value = textBox.getTextBox()
                            .getText();
                        if (value.equalsIgnoreCase("")) continue;
                        if (s1.equalsIgnoreCase(label)) {
                            if (s2.equalsIgnoreCase("I")) rewardInstance.addProperty(label, Integer.parseInt(value));
                            if (s2.equalsIgnoreCase("S")) rewardInstance.addProperty(label, value);
                    ...
                    CCubesCore.logger.log(
                        Level.ERROR,
                        "An error has occured while saving this reward! Please make sure all fields are correctly filled!");
                }

            }
        }
        this.prevStage[3] = this.prevStage[3].substring(0, this.prevStage[3].indexOf(":") + 1)
            + rewardInstance.toString();
        try {
            FileOutputStream outputStream = new FileOutputStream(file);
            OutputStreamWriter writer = new OutputStreamWriter(outputStream);
            writer.append(gson.toJson(fileJson));
    ...
    public String[] getsElementsFromReward(String reward) {
        if (reward.equalsIgnoreCase("Item")) return ItemPart.elements;
        else if (reward.equalsIgnoreCase("Block")) return OffsetBlock.elements;
        else if (reward.equalsIgnoreCase("Message")) return MessagePart.elements;
        else if (reward.equalsIgnoreCase("Command")) return CommandPart.elements;
        else if (reward.equalsIgnoreCase("Entity")) return EntityPart.elements;
        else if (reward.equalsIgnoreCase("Experience")) return ExpirencePart.elements;
        else if (reward.equalsIgnoreCase("Potion")) return PotionPart.elements;
        else if (reward.equalsIgnoreCase("Schematic")) return new String[] { "fileName:S", "delay:I" };
        else if (reward.equalsIgnoreCase("Sound")) return SoundPart.elements;
        else if (reward.equalsIgnoreCase("Chest")) return ChestChanceItem.elements;
        return new String[0];
    }
