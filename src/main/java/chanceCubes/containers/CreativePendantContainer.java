...
public class CreativePendantContainer extends Container {
    ...
    private IInventory pendantSlot = new InventoryBasic("CreativePendant", true, 1) {
        ...
        public boolean isItemValidForSlot(int slot, ItemStack stack) {
            return stack.getItem() instanceof ItemChanceCube || stack.getItem()
                .equals(CCubesBlocks.chanceIcosahedron);
        }
    };
    ...
    public CreativePendantContainer(InventoryPlayer player, World world) {
        ...
        for (int i = 0; i < 9; i++) this.addSlotToContainer(new Slot(player, i, 8 + i * 18, 142));
        ...
        this.addSlotToContainer(new Slot(pendantSlot, 0, 80, 50) {
            ...
            public boolean isItemValid(ItemStack stack) {
                if (stack.getItem() instanceof ItemChanceCube) return true;
                return false;
            }
        });
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slot) {
        ...
        if (slotObject != null && slotObject.getHasStack()) {
            ...
            if (slot > 35) {
                if (!this.mergeItemStack(stackInSlot, 0, 36, true)) return null;
            } else if (stack.getItem()
                .equals(CCubesBlocks.chanceCube)
                || stack.getItem()
                    .equals(CCubesBlocks.chanceIcosahedron)) {
                        if (!this.mergeItemStack(stackInSlot, 36, 37, true)) return null;
                    }
