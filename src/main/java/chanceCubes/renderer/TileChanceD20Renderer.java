...
public class TileChanceD20Renderer extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tileEntity, double posX, double posY, double posZ, float partialTick) {
        ...
        float wave = stage == 0 ? MathHelper.sin(
            (((tileEntity.getWorldObj()
                .getTotalWorldTime() % (HOVER_SPEED * 1000F) + partialTick) / (HOVER_SPEED * 1000F))
                + random.nextFloat()) * 360F)
            : ((stage + partialTick) / 10f);
        float rot = d20.rotation + (d20.rotationDelta * partialTick);
        float color = (tileEntity.getWorldObj()
            .getTotalWorldTime() % BASE_COLOR_SPEED + partialTick) / BASE_COLOR_SPEED;
        ...
        float f1 = ((float) tileEntity.getWorldObj()
            .getTotalWorldTime() % 750 + partialTick) / 750.0F;
        float f2 = 0F;
