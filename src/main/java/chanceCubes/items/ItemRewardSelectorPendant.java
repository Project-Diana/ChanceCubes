...
public class ItemRewardSelectorPendant extends BaseChanceCubesItem {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (player.isSneaking() && world.isRemote) FMLCommonHandler.instance()
            .showGuiScreen(new RewardSelectorPendantGui(player, stack));
        return stack;
    }
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (stack.getTagCompound() != null && stack.getTagCompound()
            ...
            if (world.getBlock(x, y, z)
                .equals(CCubesBlocks.chanceCube)) {
                world.setBlockToAir(x, y, z);
                IChanceCubeReward reward = ChanceCubeRegistry.INSTANCE.getRewardByName(
                    stack.getTagCompound()
                        .getString("Reward"));
                if (reward != null) reward.trigger(world, x, y, z, player);
                else player.addChatMessage(new ChatComponentText("That reward does not exist for this cube!"));
            } else if (world.getBlock(x, y, z)
                .equals(CCubesBlocks.chanceGiantCube)) {
                    TileEntity ent = world.getTileEntity(x, y, z);
                    if (ent == null || !(ent instanceof TileGiantCube)) return false;
                    TileGiantCube giant = (TileGiantCube) ent;
                    IChanceCubeReward reward = GiantCubeRegistry.INSTANCE.getRewardByName(
                        stack.getTagCompound()
                            .getString("Reward"));
                    if (reward != null)
