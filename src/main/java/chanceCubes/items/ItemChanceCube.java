...
public class ItemChanceCube extends ItemBlock {
    ...
    public void setChance(ItemStack stack, int chance) {
        if (chance > 100 || chance < -101) chance = -101;
    ...
    public int getChance(ItemStack stack) {
        if (stack.stackTagCompound == null) return -101;
    ...
    public String getChanceAsStringValue(ItemStack stack) {
        if (stack.stackTagCompound == null) return "Random";
        return stack.stackTagCompound.hasKey("Chance") ? stack.stackTagCompound.getInteger("Chance") == -101 ? "Random"
            : "" + stack.stackTagCompound.getInteger("Chance") : "Random";
    }
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean bool) {
        if (!stack.getItem()
            .equals(CCubesBlocks.chanceCubeDispenser)) {
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (te != null) {
            if (te instanceof TileChanceCube) {
                ...
                if (chance != -101) ((TileChanceCube) te).setChance(chance);
            } else if (te instanceof TileChanceD20) {
                int chance = this.getChance(stack);
                if (chance != -101) ((TileChanceD20) te).setChance(chance);
            }
        }
