        ...
        for (int xx = 0; xx < 2; xx++) {
            for (int zz = -4; zz < 5; zz++) {
                if (!skip) {
                    ...
                    if (!world.getBlock(xxx, y, pz + zz)
                        .isOpaqueCube()
                        && !world.getBlock(xxx, y + 1, pz + zz)
                            .isOpaqueCube()
                        && !world.getBlock(xxx, y + 2, pz + zz)
                            .isOpaqueCube()) {
                        creeper = new EntityCreeper(world);
                        creeper.setLocationAndAngles(xxx, y, pz + zz, xx == 1 ? 90 : -90, 0);
                        if (rand.nextInt(10) == 1) creeper.getDataWatcher()
                            .updateObject(17, Byte.valueOf((byte) 1));
        ...
        for (int xx = -4; xx < 5; xx++) {
            for (int zz = 0; zz < 2; zz++) {
                if (!skip) {
                    ...
                    if (world.getBlock(px + xx, y, zzz)
                        .equals(Blocks.air)
                        && world.getBlock(px + xx, y + 1, zzz)
                            .equals(Blocks.air)
                        && world.getBlock(px + xx, y + 2, zzz)
                            .equals(Blocks.air)) {
                        creeper = new EntityCreeper(world);
