    ...
    public void trigger(World world, int x, int y, int z, EntityPlayer player) {
        ...
        for (int xx = 0; xx < 2; xx++) {
            for (int zz = -4; zz < 5; zz++) {
                ...
                if (!world.getBlock(xxx, y, z + zz)
                    .isOpaqueCube()
                    && !world.getBlock(xxx, y + 1, z + zz)
                        .isOpaqueCube()
                    && !world.getBlock(xxx, y + 2, z + zz)
                        .isOpaqueCube()) {
        ...
        for (int xx = -4; xx < 5; xx++) {
            for (int zz = 0; zz < 2; zz++) {
                ...
                if (world.getBlock(x + xx, y, zzz)
                    .equals(Blocks.air)
                    && world.getBlock(x + xx, y + 1, zzz)
                        .equals(Blocks.air)
                    && world.getBlock(x + xx, y + 2, zzz)
                        .equals(Blocks.air)) {
                    enderman = new EntityEnderman(world);
