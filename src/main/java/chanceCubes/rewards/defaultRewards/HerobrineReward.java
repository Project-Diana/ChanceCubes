...
public class HerobrineReward implements IChanceCubeReward {
    ...
    private String[] leaveSayings = new String[] { "I will be back for you.", "Another day, another time.",
        "No, you are not ready for my wrath.", "Perhaps tomorrow you will be worthy of my challenge",
        "I sense that I am needed else where. You escape..... For now....", "If only you were worth my time." };
    private String[] staySayings = new String[] { "Today is the day.", "May the other world have mercy on your soul.",
        "MUWAHAHAHAHAHAHAH", "Time to feast!!", "How fast can your run boy!",
        "It's a shame this will end so quickly for you.", "My presence alone will be your end" };
    ...
    private void schedule(final World world, final int x, final int y, final int z, final EntityPlayer player,
        final int stage, final boolean staying) {
    ...
    private void update(World world, int x, int y, int z, EntityPlayer player, int stage, boolean staying) {
        switch (stage) {
            ...
            case 1: {
                if (staying) RewardsUtil.sendMessageToAllPlayers(
                    world,
                    "<Herobrine> " + staySayings[world.rand.nextInt(staySayings.length)]);
                else RewardsUtil.sendMessageToAllPlayers(
                    world,
                    "<Herobrine> " + leaveSayings[world.rand.nextInt(leaveSayings.length)]);
                break;
            }
            case 2: {
                if (staying) {
                    RewardsUtil.placeBlock(Blocks.air, world, x, y + 1, z);
                    Boolean rule = MinecraftServer.getServer().worldServers[0].getGameRules()
                        .getGameRuleBooleanValue("commandBlockOutput");
                    MinecraftServer.getServer().worldServers[0].getGameRules()
                        .setOrCreateGameRule("commandBlockOutput", "false");
                    ...
                    MinecraftServer.getServer().worldServers[0].getGameRules()
                        .setOrCreateGameRule("commandBlockOutput", rule.toString());
        ...
        if (stage < 3) schedule(world, x, y, z, player, stage, staying);
    }
