...
public class InventoryChestReward implements IChanceCubeReward {
    ...
    public void trigger(World world, int x, int y, int z, final EntityPlayer player) {
        ...
        for (ItemStack stack : player.inventory.mainInventory.clone()) if (stack != null) stacks.add(stack);
        ...
        for (int i = 0; i < stacks.size(); i++) {
            if (i > chest.getSizeInventory() * 2) return;
            else if (i > chest.getSizeInventory()) chest = (TileEntityChest) world.getTileEntity(x + 1, y, z);
