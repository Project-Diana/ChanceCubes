...
public class CustomUserReward implements IChanceCubeReward {
    ...
    public CustomUserReward(EntityPlayer player) {
        if (!CCubesSettings.userSpecificRewards) return;

        JsonElement users;
        try {
            users = HTTPUtil
                .getWebFile("https://api.theprogrammingturkey.com/chance_cubes/custom_rewards/UserList.json");
        } catch (Exception e) {
        ...
        for (JsonElement user : users.getAsJsonArray()) {
            if (user.getAsJsonObject()
                .get("UUID")
                .getAsString()
                .equalsIgnoreCase(uuidTemp.toString())) {
                userName = user.getAsJsonObject()
                    .get("Name")
                    .getAsString();
                uuid = uuidTemp;
                type = user.getAsJsonObject()
                    .get("Type")
                    .getAsString();
            }
        }
        ...
        try {
            userRewards = HTTPUtil.getWebFile(
                "https://api.theprogrammingturkey.com/chance_cubes/custom_rewards/users/" + userName + ".json");
        } catch (Exception e) {
        ...
        for (Entry<String, JsonElement> reward : userRewards.getAsJsonObject()
            .entrySet()) {
            customRewards.add(
                CustomRewardsLoader.instance.parseReward(reward)
                    .getKey());
        }

        ChanceCubeRegistry.INSTANCE.registerReward(this);
        player.addChatMessage(
            new ChatComponentText("Seems you have some custom Chance Cubes rewards " + this.userName + "...."));
    ...
    public void trigger(final World world, final int x, final int y, final int z, final EntityPlayer player) {
        if (!UsernameCache.getLastKnownUsername(uuid)
            .equalsIgnoreCase(player.getCommandSenderName())) {
            player.addChatMessage(
                new ChatComponentText("Hey you aren't " + this.userName + "! You can't have their reward! Try again!"));
        ...
        player.addChatMessage(
            new ChatComponentText("Selecting best (possibly deadly) reward for " + this.type + " " + this.userName));
    ...
    public void triggerAcutalReward(World world, int x, int y, int z, EntityPlayer player) {
        this.customRewards.get(world.rand.nextInt(this.customRewards.size()))
    ...
    public UUID getPlayerUUID(String username) {
        for (Map.Entry<UUID, String> entry : UsernameCache.getMap()
            .entrySet()) {
            if (entry.getValue()
