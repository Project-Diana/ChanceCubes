...
public class ThrownInAirReward implements IChanceCubeReward {
    ...
    public void trigger(World world, int x, int y, int z, final EntityPlayer player) {
        ...
        for (int yy = 0; yy < 40; yy++) for (int xx = -1; xx < 2; xx++)
            for (int zz = -1; zz < 2; zz++) RewardsUtil.placeBlock(Blocks.air, world, px + xx, py + yy, pz + zz);
        ...
        Task task = new Task("Item_Of_Destiny_Reward", 5) {
            ...
            public void callback() {
                ...
                ((EntityPlayerMP) player).playerNetServerHandler.sendPacket(
                    new S12PacketEntityVelocity(player.getEntityId(), player.motionX, player.motionY, player.motionZ));
            }
        };
