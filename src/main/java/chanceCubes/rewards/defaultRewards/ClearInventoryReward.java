...
public class ClearInventoryReward implements IChanceCubeReward {
    ...
    public void trigger(World world, int x, int y, int z, final EntityPlayer player) {
        ...
        for (int slotNum = 0; slotNum < player.inventory.getSizeInventory(); slotNum++) {
            if (player.inventory.getStackInSlot(slotNum) != null && player.inventory.getStackInSlot(slotNum)
                .getItem() instanceof ItemChanceCube) cubes = true;
            else player.inventory.setInventorySlotContents(slotNum, null);
        }
        ...
        if (cubes) player.addChatMessage(new ChatComponentText("Don't worry, I left the cubes for you!"));
