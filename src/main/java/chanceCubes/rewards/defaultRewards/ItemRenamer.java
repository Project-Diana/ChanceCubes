...
public class ItemRenamer implements IChanceCubeReward {
    ...
    public void trigger(World world, int x, int y, int z, EntityPlayer player) {
        ...
        for (ItemStack stack : player.inventory.mainInventory) if (stack != null) stacks.add(stack);

        for (ItemStack stack : player.inventory.armorInventory) if (stack != null) stacks.add(stack);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            stacks.get(rand.nextInt(stacks.size()))
                .setStackDisplayName(newName);
        }
