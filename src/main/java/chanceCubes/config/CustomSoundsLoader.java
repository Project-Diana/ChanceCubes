...
public class CustomSoundsLoader {
    ...
    public void addCustomSounds() {
        ...
        for (File f : new File(folder.getAbsolutePath() + "/Sounds")
        ...
        customsSounds.add(
            new CustomFile(
                "assets/minecraft",
                FileUtil.writeToFile(folder.getAbsolutePath() + "/Sounds/sounds.json", gson.toJson(root))));// add
                                                                                                            // record
                                                                                                            // .ogg
    }
    ...
    public void assemble() {
        ...
        if (FMLCommonHandler.instance()
            ...
            .isClient()) {
            try {
                if (defaultResourcePacks == null) defaultResourcePacks = ReflectionHelper.getPrivateValue(
                    Minecraft.class,
                ...
                CCubesCore.logger
                    .error("Failed to inject the resource pack for the custom sounds in the Chance Cubes rewards: ", e);
            }
        }
    }
