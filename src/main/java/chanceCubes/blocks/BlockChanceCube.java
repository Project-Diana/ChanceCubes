...
public class BlockChanceCube extends BaseChanceBlock implements ITileEntityProvider {
    ...
    public IIcon getIcon(IBlockAccess acess, int x, int y, int z, int side) {
        if (tempOverride) return this.getIcon(side, acess.getBlockMetadata(x, y, z));
        World world = null;
        if (acess instanceof World) {
            world = (World) acess;
        } else if (acess instanceof ChunkCache) {
            TileEntity te = ((ChunkCache) acess).getTileEntity(x, y, z);
            if (te == null) return this.getIcon(side, acess.getBlockMetadata(x, y, z));
        ...
        if (block instanceof BaseChanceBlock) return this.getIcon(side, acess.getBlockMetadata(x, y, z));
        if (dim == 0 && CCubesSettings.hiddenOverWorldCubes) {
            if (block.equals(Blocks.air)) return Blocks.stone.getIcon(side, acess.getBlockMetadata(x, y, z));
            return world.getBlock(x, y - 1, z)
                .getIcon(side, acess.getBlockMetadata(x, y, z));
        }
        if (dim == 1 && CCubesSettings.hiddenEndCubes) {
            if (block.equals(Blocks.air)) return Blocks.end_stone.getIcon(side, acess.getBlockMetadata(x, y, z));
            return world.getBlock(x, y - 1, z)
                .getIcon(side, acess.getBlockMetadata(x, y, z));
        }
        if (dim == -1 && CCubesSettings.hiddenNetherCubes) {
            if (block.equals(Blocks.air)) return Blocks.netherrack.getIcon(side, acess.getBlockMetadata(x, y, z));
            return world.getBlock(x, y - 1, z)
                .getIcon(side, acess.getBlockMetadata(x, y, z));
        }
    ...
    public IIcon getIcon(int side, int meta) {
        if (CCubesSettings.hasHolidayTexture) return this.specialIcons[side];
        else return this.icons[side];
    }
