...
public class BlockCubeDispenser extends BaseChanceBlock implements ITileEntityProvider {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int meta, float p_149727_7_,
        ...
        if (!(world.getTileEntity(x, y, z) instanceof TileCubeDispenser)) return false;
        ...
        if (player.isSneaking()) {
            ...
            if (player.inventory.getCurrentItem() != null && Block.getBlockFromItem(
                player.inventory.getCurrentItem()
                    .getItem())
                .equals(te.getCurrentBlock(world.getBlockMetadata(x, y, z))))
    ...
    public void onBlockClicked(World world, int x, int y, int z, EntityPlayer player) {
        ...
        if (!(world.getTileEntity(x, y, z) instanceof TileCubeDispenser)) return;
        TileCubeDispenser te = (TileCubeDispenser) world.getTileEntity(x, y, z);
    ...
    public IIcon getIcon(int side, int meta) {
        if (side == 0 || side == 1) return this.top;
        return this.side;
    }
